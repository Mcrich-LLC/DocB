...
## Showing Symbol Badges

DocCKit can render Xcode-style symbol badges for DocC references. The reference list rows can show class, struct, protocol, enum, method, property, article, and custom image badges.

To show badges in an article's related links and other reference lists, enable `showsReferenceSymbolBadges` like so:

```swift
DocCArticleView(
    page: page,
    navigator: navigator,
    showsReferenceSymbolBadges: true
)
```

You can also render a badge directly:
...
DocCSymbolBadge(
    ...
    docCSite: client.source
)
```

For custom index icons, use the icon identifier from `DocCIndex.InterfaceLanguage/icon` and the archive identifier from `DocCIndex/includedArchiveIdentifiers`:

```swift
DocCSymbolBadge(
    symbolKind: SidebarSearchSymbolKind(interfaceLanguage: interfaceLanguage),
    size: 22,
    customImageIdentifier: interfaceLanguage.icon,
    docCSite: client.source,
    archiveIdentifier: client.source.index.includedArchiveIdentifiers?.first
)
```

If the custom image cannot be resolved or loaded, `DocCSymbolBadge` falls back to the standard badge for the resolved symbol kind.
...
> [!TIP]
> [DocPage](https://github.com/Mcrich-LLC/DocPage) is a super easy to use tool we made that can help you convert a signle markdown file to DocC JSON.
