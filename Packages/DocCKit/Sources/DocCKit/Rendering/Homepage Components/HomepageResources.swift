...
struct HomepageResources: View {
    ...
    func item(_ item: HomepageParser.Resource) -> some View {
        VStack {
            if let imageId = item.image, let imageUrl = DocCAssetResolver.fetchPhotoVideoURL(for: imageId, references: homepage.references, colorScheme: colorScheme, docCSite: nil) {
